...
contract NFTPayment is ERC721, Ownable, ReentrancyGuard {
    ...
    /// @dev Price feed for the payment token (BTC/ETH) - sepolia
    ...
    event MintedWithBTC(address indexed to, uint256 tokenId, uint256 amountPaidInETH);
    ...
    constructor(
        ...
        address btcEthPriceFeed_
    ) ERC721(name_, symbol_) Ownable(initialOwner) {
        if (paymentToken_ == address(0)) revert ZeroAddress();
        if (initialOwner == address(0)) revert ZeroAddress();
        if (btcEthPriceFeed_ == address(0)) revert ZeroAddress();
        ...
        saleActive = true;
        btcEthPriceFeed = AggregatorV3Interface(btcEthPriceFeed_);
    }
    ...
    /// @notice Mint NFT paying with BTC
    /// @param to Address to receive the NFT
    function mintWithBTC(address to) external payable nonReentrant {
        ...
        if (btcEthPrice <= 0) revert InsufficientPayment();
        uint256 calculatedPriceInETH = (priceInBTC * uint256(btcEthPrice)) / 1e18;
        if (msg.value < calculatedPriceInETH) revert InsufficientPayment();
        ...
        emit MintedWithBTC(to, tokenId, calculatedPriceInETH); // amount paid in ETH
    }
    ...
    /// @notice Calculate BTC price equivalent to given ETH price using price feed
    /// @param ethPrice Price in ETH (wei)
    /// @return btcPrice Equivalent price in BTC (wei)
    function calculateBTCPriceFromETH(uint256 ethPrice) public view returns (uint256 btcPrice) {
        int256 btcEthPrice = getBTCEthPrice();
        if (btcEthPrice <= 0) revert InsufficientPayment();
        // If 1 BTC = btcEthPrice ETH, then ethPrice ETH = (ethPrice * 1e18) / btcEthPrice BTC
        btcPrice = (ethPrice * 1e18) / uint256(btcEthPrice);
    }

    /// @notice Owner: Update prices for both payment methods
    /// @dev BTC price should be calculated to be equivalent to ETH price using price feed
    function setPrices(uint256 newPriceInToken, uint256 newPriceInETH, uint256 newPriceInBTC) external onlyOwner {
        priceInToken = newPriceInToken;
        priceInETH = newPriceInETH;
        priceInBTC = newPriceInBTC;
        emit PriceUpdated(newPriceInToken, newPriceInETH, newPriceInBTC);
    }

    /// @notice Owner: Update prices, automatically calculating BTC price from ETH price
    /// @param newPriceInToken New price in payment token
    /// @param newPriceInETH New price in ETH
    function setPricesWithAutoBTC(uint256 newPriceInToken, uint256 newPriceInETH) external onlyOwner {
        uint256 newPriceInBTC = calculateBTCPriceFromETH(newPriceInETH);
        priceInToken = newPriceInToken;
        priceInETH = newPriceInETH;
        priceInBTC = newPriceInBTC;
        emit PriceUpdated(newPriceInToken, newPriceInETH, newPriceInBTC);
    ...
    /// @notice Get current price in ETH for 1 BTC
